...
from price_parser_uniswap.env import UNI_CLI_PATH, JSON_RPC_PROVIDER
...
async def parse_path_price(
    ...
    for percent, perc_path in path.pools:
        if is_buy:
...
async def main():
    connection = web3.AsyncWeb3(web3.AsyncHTTPProvider(JSON_RPC_PROVIDER))
    ...
    while True:
        for instrument in REQUIRED_INSTRUMENTS:
            if await locker_db.is_lock(instrument.value):
                continue
            quote_in = await path_db_ins.get_path(instrument, QuoteType.exactIn)
            quote_out = await path_db_ins.get_path(instrument, QuoteType.exactOut)
            if quote_in and quote_out:
                await locker_db.lock_action(instrument.value, 1000)
                await parse_path_price(quote_in, connection)
                await parse_path_price(quote_out, connection)
