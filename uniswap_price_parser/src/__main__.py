...
from obs_shared.connection import PathRConnection, SyncRConnection, RMQConnection, ActiveSettingsExchangeRConnection, \
    InfoSettingsRConnection, PriceRConnection
from obs_shared.static import uni_v3_quoter_abi
from obs_shared.types import CalculationPrice, TokenRow, PriceRow

from src.const import UNI_V3_QUOTER_ADDRESS, CHAIN
from src.env import PATH_R_HOST, PATH_R_DB, PATH_R_PASSWORD, SYNC_R_HOST, SYNC_R_DB, SYNC_R_PASSWORD, SENDER_RMQ_QUE, \
    SENDER_RMQ_PASSWORD, SENDER_RMQ_USER, SENDER_RMQ_HOST, SETTINGS_R_HOST, SETTINGS_R_DB, SETTINGS_R_PASSWORD, \
    INFO_R_HOST, INFO_R_DB, INFO_R_PASSWORD, JSON_RPC_PROVIDER, PRICE_R_HOST, PRICE_R_DB, PRICE_R_PASSWORD
...
price_rconn: PriceRConnection = PriceRConnection(PRICE_R_HOST, PRICE_R_DB, PRICE_R_PASSWORD)
...
def _calc_price_data(pair_symbol: str, type_: str):
    ...
    if not sync_rconn.is_lock(sync_key):
        ...
        if path is None:
            return None
        actual_price = PriceRow(price_rconn.get_exchange_pair_price(NAME, pair_symbol))
        is_buy = type == "exactIn"
        ...
        calc_entity = CalculationPrice(
            ...
            symbol=pair_symbol,
            exchange=NAME,
            price=actual_price.to_printable_str(),
            buy=is_buy
        )
        price_rconn.set_exchange_pair_price(NAME, pair_symbol, str(actual_price))
